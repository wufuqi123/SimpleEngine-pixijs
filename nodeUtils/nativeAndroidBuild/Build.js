...
let outDir = "./native/android/app/src/main/assets/web";
...
bundler.on('buildEnd', () => {
    //写入andriod  config
    WriteConfig.write({ web_url: "file:///android_asset/web/index.html" }, () => {
        processCmd.exec("npm run android:native:build").on("close", (close) => {
            console.log("打包完成");
        }).stdout.on("data", (data) => {
            console.log(data);
        })
