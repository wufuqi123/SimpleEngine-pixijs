...
import { AudioContextGroup } from "./AudioContextGroup";

export class AudioContextGroupManager {
  private mGroupMap:Map<string,AudioGroupInterface> = new Map();
  private constructor(){}
  protected static instance: AudioContextGroupManager;
  public static getInstance() {
    if (!this.instance) {
      this.instance = new AudioContextGroupManager();
    }
    return this.instance;
  }
  public getAudioArr():Array<AudioInterface>{
    let arr = new Array();
    this.mGroupMap.forEach(a=>arr.push(a))
    return arr;
  }
  /**
   * 获取组
